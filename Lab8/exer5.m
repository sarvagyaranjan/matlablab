...
[b, a] = cheby1(N, rp, w, 's');
[H, W] = freqs(b,a);
subplot(4,2,1);plot(W/(2*pi),20*log10(abs(H))),grid on;
...
subplot(4,2,2);plot(wbd/pi,20*log10(abs(Hbd))),grid on;
...
[H2, W2] = freqs(b2,a2);
subplot(4,2,3);plot(W2/(2*pi),20*log10(abs(H2))),grid on;
title('Frequency Response');
xlabel(' Frequency in Hz '),ylabel(' Magnitude in dB ');
...
subplot(4,2,4);plot(wbd2/pi,20*log10(abs(Hbd2))),grid on;
ylabel('Magnitiude in dB');
xlabel('Frequency in pi units');
...
[H3, W3] = freqs(b3,a3);
subplot(4,2,5);plot(W3/(2*pi),20*log10(abs(H3))),grid on;
title('Frequency Response');
xlabel(' Frequency in Hz '),ylabel(' Magnitude in dB ');
...
subplot(4,2,6);plot(wbd3/pi,20*log10(abs(Hbd3))),grid on;
ylabel('Magnitiude in dB');
xlabel('Frequency in pi units');
...
[H4, W4] = freqs(b4,a4);
subplot(4,2,7);plot(W4/(2*pi),20*log10(abs(H4))),grid on;
title('Frequency Response');
xlabel(' Frequency in Hz '),ylabel(' Magnitude in dB ');
disp(' the order and cut off frequency in Hz are '),disp( N3),disp( w3/(2*pi));
[bb_z4, ab_z4] = bilinear(b4,a4,1/T);
[Hbd4,wbd4] =  freqz(bb_z4,ab_z4);
subplot(4,2,8);plot(wbd4/pi,20*log10(abs(Hbd4))),grid on;
ylabel('Magnitiude in dB');
xlabel('Frequency in pi units');
